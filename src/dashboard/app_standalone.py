...
import sqlite3
import os
import faiss  # ← moved to top level

st.set_page_config(page_title="PaperLens", page_icon="🔬", layout="wide")

st.title("🔬 PaperLens")
st.caption("Semantic search over arXiv papers · cs.AI · cs.LG · cs.CL")
...
if query:
    with st.spinner("Searching papers... (first load takes ~30s)"):
    ...
    for score, idx in zip(scores[0], indices[0]):
        cur.execute(
            "SELECT id, title, abstract, authors, published FROM papers WHERE rowid=?",
            (int(idx) + 1,)
        )
        row = cur.fetchone()
        if row:
    ...
    st.info("👆 Type a query above to search arXiv papers.")